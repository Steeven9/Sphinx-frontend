...
// Uncomment this and delete next line when the API route is ready
// const fetchOwnerUrl = host + '/guests/houses' + params.get('owner');
...
const ScenesPanel = () => {
        ...
        const [scenesValidation, setScenesValidation] = useState(false);
        const [hasRooms, setHasRooms] = useState(false);
        const [hasDevices, setHasDevices] = useState(false);
        const [errorMessage, setErrorMessage] = useState(false);
        const ColorCircularProgress = withStyles({ root: { color: '#580B71' } })(CircularProgress);

        // Sets the fetchUrl route according to the requested view
        useEffect(() => {
            if (path[1] === 'shared') {
        ...
        // Lets the CircularProgress run until all validations are finished
        useEffect(() => {
            if (initialValidation && scenesValidation) {
                setIsLoading(false);
            }
        }, [initialValidation, scenesValidation]);

        // Corroborates if user has rooms and devices created and disables the add button
        useEffect(() => {
            const method = 'GET';
            const headers = {
        ...
        useEffect(() => {
            ...
            try {
                const fetchData = async () => {
                    ...
                    const fetchedScenes = await res.json();
        ...
        // Manages error the messages to be shown to the user
        useEffect(() => {
            if (!isLoading && initialValidation) {
                if (!hasRooms) {
                    setErrorMessage(
                        <>
                    ...
                    setErrorMessage(
                        <span>You haven't added any scenes yet. Please add a new one.</span>
                    );
                }
            }
        }, [initialValidation, isLoading, isDataFound, hasRooms, hasDevices]);

        // if (hasRooms && !isLoading) {
        //     if (!hasDevices) {
        //         return (
        //             <>
        //                 <span>You haven't added any devices yet.&nbsp;</span>
                        ...
                        <article className="row row-scene row-scene-custom">
                            <div id="content">
                                ...
                                >
                                    ...
                                    >
                                        <p className={(isNetworkError && isShared) ? 'error-message' : 'hidden'}>{errorMessage}</p>
                                    </div>
